...
if(count($hostParams) == 3){ //3段式URL，普通web地址
    ...
    header('Location:http://www.pinganfang.com');
    exit();
}
...
$versionMap = json_decode(file_get_contents($versionFile),'true');
require_once __DIR__.'/version.php';
$version = PAFVersion::getVersion($versionMap);

//定义一系列系统常量
define('PAF_ENV_NAME',          'ga');                                              //环境名
define('PAF_VERSION',           $version);                                          //cloud名
define('PAF_SYS_PATH',          $vendorPath.'/v'.$vendorVersion);                   //vendor路径
define('PAF_APP_PATH',          $releasePath.'/'.$appName.'/'.$version.'/app');     //APP路径
define('PAF_STORAGE_PATH',      $storagePath.'/'.$appName.'/'.$version);            //STORAGE路径
define('PAF_ENV_CONFIG_PATH',   $configPath.'/'.PAF_ENV_NAME.'/'.$appName);         //环境变量配置路径
define('PAF_LOG_PATH',          $logPath.'/'.$httpHost);                            //日志路径

if(!is_dir($releasePath.'/'.$appName.'/'.$version)){
    header('Location:http://www.pinganfang.com');
    exit();
}

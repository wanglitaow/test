...
require_once(getenv('PATH_VENDOR') . '/laravel/v5.0.14/vendor/autoload.php');

$appName = isset($_SERVER['APP_NAME']) ? $_SERVER['APP_NAME'] : 'gold';
$versionPath = env('PATH_VERSION');
$versionFile = sprintf('%s.json', $appName);
$file = $versionPath . DIRECTORY_SEPARATOR . $versionFile;
$version = json_decode(file_get_contents($file, true));

EnvLoader::setEnvironmentVariable('CURRENT_VERSION', $version->ga);

return env('PATH_APP') . "/$appName/" . $version->ga;
